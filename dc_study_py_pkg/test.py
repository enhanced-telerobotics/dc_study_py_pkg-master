...
class SimStudyController(Node):
    ...
    async def run_state(self, state):
        ...
        if state == 'training_task':
            for curr_trial_index in range(trial_limit):
...
def main(args=None):
    ...
    try:
        # Use asyncio.run() to handle the main coroutine
        asyncio.run(node.run_study())
    except KeyboardInterrupt:
        node.get_logger().info('Simulation interrupted by user.')
    finally:
        rclpy.shutdown()
        node.get_logger().info('Simulation shut down.')
